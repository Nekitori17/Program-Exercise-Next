try:
  ...
  with open("./build/1_sum_min_max.out", "w") as out:
    out.writelines([
      ...
      f"Trung bình cộng các dòng là: 1 = {sum(list_hang_1) / len(list_hang_1)} 2 = {sum(list_hang_2) / len(list_hang_2)}" + "\n",
      f"Các số được sắp xếp: {sorted(list_hang_1)} | {sorted(list_hang_1)}" + "\n",
